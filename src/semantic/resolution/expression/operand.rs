...
pub(super) type R = Result<(QualifiedType, ExpressionKind), Diagnosis>;

type Operands<'s, const N: usize> = Loan<'s, Sema, ExpressionId, ResolvedExpression, N>;

pub(crate) fn operands<'s, const N: usize>(
    sema: &'s mut Sema,
...
pub(super) fn is_bit_field(sema: &Sema, sym: Option<SymbolId>) -> bool {
    let Some(id) = sym else { return false };
    let sym = id.resolve(sema);
    sym.kind == SymbolKind::Member && sym.value.is_some()
}

pub(super) fn check_assignable(lhs: &ResolvedExpression) -> Result<(), Diagnosis> {
