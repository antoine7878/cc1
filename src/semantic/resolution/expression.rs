...
use crate::ast::{
    BinaryOp, Expression, ExpressionId, ExpressionNode, MemberOp, Name, Storage, Tag, Type, UnaryOp, Value,
};
...
pub(crate) type Operands<'s, const N: usize> = Loan<'s, Sema, ExpressionId, ResolvedExpression, N>;

pub(crate) fn operands<'s, const N: usize>(sema: &'s mut Sema, nodes: [&ExpressionNode; N]) -> Result<Operands<'s, N>, Diagnosis> {
