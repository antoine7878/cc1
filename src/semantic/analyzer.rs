...
impl Analyzer {
    ...
    pub fn check_constants(ctx: Context) -> Context {
        with_sema(ctx, eval::check_constants)
    }

    pub fn mark_uses(ctx: Context) -> Context {
        with_sema(ctx, mark_uses)
    }

    pub fn finish(ctx: Context) -> Context {
        with_sema(ctx, |sema, _| finish_externals(sema))
    }
}

fn with_sema(mut ctx: Context, pass: impl FnOnce(&mut Sema, &Context)) -> Context {
