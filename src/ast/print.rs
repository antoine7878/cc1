...
use crate::ast::{
    ...
    StructDeclaration, StructDeclarator, TranslationUnitNode, Type, TypeSpecifier, declaration,
};
...
impl<'a, W: Write> AstPrinter<'a, W> {
    ...
    fn print_external_declaration_node(&mut self, node: &ExternalDeclarationNode, is_last: bool) -> io::Result<()> {
        match &node.decl {
            ExternalDeclaration::Declaration(declaration) => self.print_declaration_node(declaration, is_last),
            _ => unimplemented!(),
    ...
    fn print_init_declaration_node(&mut self, node: &InitDeclaratorNode, is_last: bool) -> io::Result<()> {
        self.print_node(node, is_last, |printer| {
...
// impl Context {
//     pub fn print_ast<>(&self, ) -> std::io::Result<()> {
//         let node = &self.ast;
