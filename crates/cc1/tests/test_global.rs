...
use cc1::context::{Context, ctx, install};
use cc1::semantic::{Sema, install as install_sema, sema};
use cc1::target::I386;
...
fn installs_are_isolated_per_thread() {
    install(named("outer.c"));
    let inner = thread::spawn(|| {
        install(named("inner.c"));
        ctx().file_name.clone()
    })
    .join()
    .unwrap();
...
fn string_ids_resolve_through_the_installed_context() {
    ...
    let id = context.arenas.names.alloc("hello".to_string());
    install(context);
