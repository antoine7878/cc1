...
accept!(tag_mutually_recursive, "struct A { struct B *b; }; struct B { struct A *a; };");

reject!(tag_redefinition, "struct S { int a; }; struct S { int b; };");

reject!(tag_kind_mismatch, "struct S { int a; }; union S { int c; };");

accept!(tag_reference_in_inner_scope, "struct S { int a; }; void f(void) { struct S s; }");
...
accept!(unsigned_bit_field_width_at_the_type_width, "struct S { unsigned a : 32; };");
accept!(bit_field_width_from_a_constant_expression, "struct S { int a : 2 + 1; };");
...
reject!(unsigned_bit_field_width_exceeding_the_type_width, "struct S { unsigned a : 33; };");
reject!(anonymous_bit_field_width_exceeding_the_type_width, "struct S { int a; int : 33; };");
...
reject!(bit_field_width_in_a_union_exceeding_the_type_width, "union U { int a : 33; };");
...
accept!(typedef_qualifier_applies_to_outer_level, "typedef int *P; const P p; int *const p;");

reject!(typedef_duplicate_qualifier, "typedef const int CI; const CI q;");

accept!(typedef_of_tag, "struct S { int a; }; typedef struct S S; S v; struct S w;");
...
accept!(enumerator_implicit_increment, "enum e { A, B, C }; int f(void) { return C; }");

accept!(enumerator_references_previous, "enum e { A = 1, B = A + 1, C = B * 2 + A };");

accept!(enumerator_negative_start, "enum e { A = -1, B, C };");

accept!(enumerator_char_constant, "enum e { A = 'a', B };");

accept!(enumerator_parenthesized, "enum e { A = (1 + 2) * 3 };");

accept!(enumerator_int_max, "enum e { M = 2147483647 };");

accept!(enumerator_sizeof_type, "enum e { A = sizeof(int), B };");

accept!(enum_tag_complete_after_definition, "enum e { A = 1 }; enum e v;");

accept!(enumerator_used_in_function, "enum e { A = 1 }; int f(void) { int x; x = A; return x; }");

accept!(enumerator_shadowed_in_inner_scope, "enum e { A = 1 }; int f(void) { int A; A = 2; return A; }");

reject!(enumerator_undeclared_reference, "enum e { A = Z };");

reject!(enumerator_references_object, "int x; enum e { A = x };");

reject!(enumerator_non_integer_constant, "enum e { A = 1.5 };");

reject!(enumerator_exceeds_int_range, "enum e { M = 2147483647, N };");

reject!(identifier_undeclared_in_expression, "int f(void) { return undeclared_thing; }");

reject!(identifier_used_before_declaration, "int f(void) { return v; }");

recover!(
    enum_recovers_after_undeclared_enumerator,
    "enum e { A = Z, B = 2, C = B + 1 }; enum e v; int f(void) { return B + C; }",
    [Diagnostic::UndeclaredIdentifier(_), Diagnostic::NonConstantExpression],
    &["B", "C"]
);

recover!(
    enum_recovers_after_non_integer_enumerator,
...
recover!(
    enum_recovers_after_out_of_range_enumerator,
    "enum h { M = 2147483647, N, O = 5 }; int g(void) { return O; }",
    [Diagnostic::EnumeratorBadValue],
    &["O"]
);
...
reject!(cast_non_immediate_float_operand_rejected, "enum e { A = (int)(1.5 + 1) };");

reject!(cast_to_pointer_rejected, "enum e { A = (int *)0 };");

reject!(cast_to_void_rejected, "enum e { A = (void)0 };");

reject!(cast_to_struct_rejected, "struct s { int a; }; enum e { A = (int)(struct s)1 };");

reject!(cast_of_object_rejected, "int x; enum e { A = (int)x };");

reject!(cast_unsigned_wraparound_exceeds_int_range, "enum e { A = (unsigned int)-1 };");
...
value!(cast_value_unsigned_int_wraps, "enum e { A = (unsigned int)-1 % 1000 };", &[("A", "295")]);
...
reject!(block_function_declaration_with_auto, "void f(void) { auto int g(void); }");

reject!(block_function_declaration_with_static, "void f(void) { static int g(void); }");

reject!(block_function_declaration_with_register, "void f(void) { register int g(); }");

reject!(nested_block_function_declaration_with_static, "void f(void) { { static int g(void); } }");

accept!(block_function_declaration_with_extern, "void f(void) { extern int g(void); }");

accept!(block_function_declaration_without_storage, "void f(void) { int g(void); }");

accept!(block_declaration_of_a_pointer_to_function, "void f(void) { auto int (*fp)(void); }");

accept!(block_typedef_of_a_function_type, "void f(void) { typedef int T(void); }");
...
reject!(function_returning_an_array_through_a_typedef, "typedef int A[3]; A f(void);");
...
accept!(function_declared_with_an_incomplete_return_type, "struct S; struct S f(void);");
...
accept!(function_prototype_then_identifier_list_definition, "int f(int); int f(a) int a; { }");

accept!(function_unspecified_then_identifier_list_definition, "int f(); int f(a) int a; { }");

accept!(function_identifier_without_declaration_is_int, "int f(int); int f(a) { }");

accept!(function_identifier_short_promotes_to_int, "int f(int); int f(a) short a; { }");

accept!(function_identifier_float_promotes_to_double, "int f(double); int f(a) float a; { }");

accept!(function_variadic_prototype_then_identifier_list_definition, "int f(int,...); int f(a) int a; { }");

accept!(function_identifier_list_definition_then_prototype, "int f(a) int a; { } int f(int);");

reject!(function_prototype_then_empty_identifier_list_definition, "int f(int); int f() { }");

reject!(function_identifier_list_definition_with_fewer_parameters, "int f(int,int); int f(a) int a; { }");

reject!(function_identifier_char_promotes_to_int, "int f(char); int f(a) char a; { }");

// ---- 6.5.2.2 an enumerated type is compatible with an integer type --------

accept!(enum_parameter_prototype_then_unspecified, "enum e { A }; int f(enum e); int f();");
...
reject!(function_identifier_list_definition_with_other_return_type, "int f(int); void f(a) int a; { }");
...
reject!(compatible_volatile_against_plain, "extern volatile int x; extern int x;");

reject!(compatible_const_pointer_against_plain, "extern int *const p; extern int *p;");

reject!(compatible_pointer_to_const_against_pointer, "extern const int *p; extern int *p;");

reject!(compatible_const_tag_against_plain, "struct S { int a; }; extern const struct S s; extern struct S s;");

reject!(compatible_const_element_against_plain, "extern const int a[10]; extern int a[10];");

// ---- 6.5.4.1 pointers to compatible types --------------------------------

accept!(compatible_pointer_to_function, "extern int (*p)(int); extern int (*p)(int);");

reject!(compatible_pointer_to_function_other_parameter, "extern int (*p)(int); extern int (*p)(char);");

reject!(compatible_pointer_to_function_other_return, "extern int (*p)(int); extern char (*p)(int);");

accept!(compatible_pointer_to_completed_tag, "struct S; extern struct S *p; struct S { int a; }; extern struct S *p;");
...
accept!(compatible_array_both_unsized, "extern const int a[]; extern const int a[10];");

reject!(compatible_array_other_size, "extern int a[10]; extern int a[11];");

accept!(compatible_array_of_array_outer_unsized, "extern int a[][4]; extern int a[3][4];");

accept!(compatible_array_of_array_outer_sized_first, "extern int a[10][4]; extern int a[][4];");

reject!(compatible_array_of_array_other_inner_size, "extern int a[3][4]; extern int a[3][5];");

accept!(compatible_pointer_to_unsized_array, "extern int (*p)[10]; extern int (*p)[];");

// ---- 6.5.2.2 an enumerated type is compatible with one integer type ------

accept!(compatible_enum_declared_twice, "enum a { X }; extern enum a v; extern enum a v;");

reject!(compatible_distinct_enums, "enum a { X }; enum b { Y }; extern enum a v; extern enum b v;");

// ---- 6.1.2.5 the character and integer types are distinct types ----------

reject!(compatible_char_against_signed_char, "extern char c; extern signed char c;");

reject!(compatible_int_against_unsigned_int, "extern int x; extern unsigned int x;");
...
reject!(incomplete_static_block_variable, "struct S; void f(void) { static struct S x; }");
...
reject!(self_referential_struct_member_no_crash, "struct W { struct W w; }; enum e { P = sizeof(struct W) };");
...
reject!(self_referential_struct_array_member_no_crash, "struct W { struct W w[3]; }; enum e { P = sizeof(struct W) };");

// ---- 6.7.1 the resulting parameter type shall be an object type ----------

reject!(incomplete_parameter_prototype, "struct S; void h(struct S p) { (void)0; }");
...
accept!(extern_incomplete_block_variable, "struct S; void f(void) { extern struct S y; }");
...
accept!(tentative_definition_completed_later, "struct S; struct S late; struct S { int a; };");
...
recover!(
    ...
    [Diagnostic::InvalidElementType(_)],
    &[]
);
