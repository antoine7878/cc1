...
stmts!(
    ...
    &[
        ...
        "#1 case Int(1)->#5",
        "#3 default->#5",
        "#5 switch int [Int(1)->#1] default=#3",
    ]
);
...
stmts!(
    ...
    &[
        ...
        "#1 case Int(1)->#4",
        "#2 continue->#6",
        "#4 switch int [Int(1)->#1] default=none",
        "#6 loop",
    ]
);
stmts!(
    ...
    &[
        ...
        "#1 case Int(1)->#5",
        "#3 loop",
        "#5 switch int [Int(1)->#1] default=none",
    ]
);
...
accept!(
    stmt_case_beyond_the_unpromoted_range,
    "void f(char c){ switch (c) { case 1: ; case 257: ; } }"
);
reject!(
    stmt_duplicate_case_after_conversion,
    "void f(void){ switch (1) { case 1: ; case 1L: ; } }"
);
accept!(
    stmt_case_inside_a_loop_inside_its_switch,
    "void f(void){ switch (1) { while (1) { case 1: ; } } }"
);
accept!(
...
accept!(
    stmt_default_inside_a_loop_inside_its_switch,
    "void f(void){ switch (1) { while (1) { default: ; } } }"
);
accept!(
    stmt_nested_switches_keep_their_own_cases,
    "void f(void){ switch (1) { switch (2) { case 1: ; } case 2: ; } }"
);
reject!(stmt_case_outside_any_switch, "void f(void){ while (1) { case 1: ; } }");
reject!(
    stmt_continue_outside_any_loop,
    "void f(void){ switch (1) { case 1: continue; } }"
);

// 6.6.4.2 The expression of each case label shall be an integral constant expression.
reject!(
    stmt_case_with_a_floating_value,
    "void f(void){ switch (1) { case 1.0: ; } }"
);
reject!(
    stmt_case_with_a_non_constant,
    "void f(void){ int x; switch (1) { case x: ; } }"
);
