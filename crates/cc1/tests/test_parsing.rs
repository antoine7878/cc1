...
accept!(typedef_redef_in_block, "typedef int T; void f(void) { typedef T T; T x; x = 1; }");

accept!(knr_definition_with_typedef_param_type, "typedef int T; f(a) T a; { return a; }");

accept!(block_typedef_no_leak, "void f(void) { typedef int T; } int T;");

reject!(nested_block_typedef_no_leak, "typedef int T; void f(void) { if (1) { typedef char U; } } U u;");
...
accept!(sizeof_shadowed_verdict, "typedef int T; int main(void) { int T; return sizeof(T); }");

accept!(local_var_shadows_typedef, "typedef int T; int main(void) { int T; T = 1; return T; }");

accept!(param_shadows_typedef, "typedef int T; T f(T T) { return T; }");

accept!(paren_expr_of_shadowed_typedef, "typedef int T; int main(void) { int T; return (T); }");

accept!(sizeof_expr_on_shadowed_typedef, "typedef int T; int main(void) { int T; return sizeof T; }");
...
accept!(member_access_named_as_typedef, "typedef int x; struct S { int x; }; void f(void) { struct S s; s.x = 1; }");

reject!(typedef_redeclared_as_var, "typedef int T; int T;");

accept!(knr_struct_def_in_declaration_list, "f(a) struct S { int x; } *a; { return 0; }");
...
accept!(typedef_after_nontypedef_decl, "typedef int T; int y; void f(void) { y = 1; }");
...
syntax!(constant_suffixes, "unsigned long f(void) { return 1ul + 2UL + 3L + 4U; }");
syntax!(constant_char, "int f(void) { return 'a'; }");
syntax!(constant_char_escape, "int f(void) { return '\\n' + '\\0' + '\\\\' + '\\''; }");
syntax!(constant_wide_char, "int f(void) { return L'a'; }");
syntax!(constant_float, "double f(void) { return 1.0 + .5 + 2. + 1e10 + 1.5e-3 + 1.0f + 1.0L; }");
...
syntax!(postfix_call_many_args, "int g(int, int, int); int f(void) { return g(1, 2, 3); }");
syntax!(postfix_call_comma_arg, "int g(int); int f(void) { return g((1, 2)); }");
syntax!(postfix_call_through_pointer, "int (*fp)(int); int f(void) { return (*fp)(1) + fp(2); }");
syntax!(postfix_member_dot, "struct S { int m; } s; int f(void) { return s.m; }");
syntax!(postfix_member_arrow, "struct S { int m; } *p; int f(void) { return p->m; }");
syntax!(postfix_member_chain, "struct A { int m; }; struct B { struct A a; } b; int f(void) { return b.a.m; }");
syntax!(postfix_inc_dec, "int f(void) { int x; x = 0; x++; x--; return x; }");

// ---- 6.3.3 unary expressions --------------------------------------------

syntax!(unary_prefix_inc_dec, "int f(void) { int x; x = 0; ++x; --x; return x; }");
syntax!(unary_address_and_deref, "int f(void) { int x; int *p; x = 0; p = &x; return *p; }");
syntax!(unary_arithmetic, "int f(void) { int x; x = 1; return +x + -x + ~x + !x; }");
syntax!(unary_sizeof_expr, "int f(void) { int x; x = 0; return sizeof x; }");
syntax!(unary_sizeof_paren_expr, "int f(void) { int x; x = 0; return sizeof (x); }");
syntax!(unary_sizeof_type, "int f(void) { return sizeof(int); }");
syntax!(unary_sizeof_ptr_type, "int f(void) { return sizeof(char *) + sizeof(int (*)(void)); }");
...
syntax!(cast_to_function_pointer, "void f(void) { int (*p)(int); p = 0; (void)(int (*)(int))p; }");
...
syntax!(binary_relational, "int f(int a, int b) { return (a < b) + (a > b) + (a <= b) + (a >= b); }");
...
syntax!(binary_precedence_mix, "int f(int a, int b, int c) { return a + b * c - a / b % c; }");
...
syntax!(storage_auto_register, "int f(void) { auto int x; register int y; x = 0; y = 0; return x + y; }");
...
syntax!(type_int_forms, "int a; signed b; signed int c; unsigned d; unsigned int e;");
syntax!(type_long_forms, "long a; long int b; signed long c; signed long int d; unsigned long e; unsigned long int f;");
syntax!(type_float_double, "float a; double b; long double c;");
syntax!(type_specifier_order_swapped, "int unsigned a; int long b; char signed c;");
...
syntax!(struct_mutually_recursive, "struct A { struct B *b; }; struct B { struct A *a; };");
syntax!(struct_nested_definition, "struct Outer { struct Inner { int x; } in; };");
syntax!(struct_multiple_declarators, "struct S { int a, b, *c, d[4]; };");
syntax!(struct_bitfields, "struct S { unsigned a : 1; signed b : 3; int c : 2; };");
syntax!(struct_anonymous_bitfield, "struct S { unsigned a : 1; unsigned : 2; int b; };");
syntax!(struct_zero_width_bitfield, "struct S { unsigned a : 1; unsigned : 0; unsigned b : 1; };");
syntax!(struct_qualified_members, "struct S { const int a; volatile int b; const volatile int c; };");
...
syntax!(qualifier_on_pointer, "int *const p = 0; const int *q; const int *const r = 0;");
...
syntax!(init_struct_nested, "struct I { int x; }; struct O { struct I i; int y; } o = { { 1 }, 2 };");
...
syntax!(abstract_function_pointer, "int f(void) { return sizeof(int (*)(void)); }");
...
syntax!(abstract_struct, "struct S { int a; }; int f(void) { return sizeof(struct S); }");

// ---- 6.6.1 labeled statements --------------------------------------------

syntax!(label_and_goto, "void f(void) { goto end; end: ; }");
syntax!(label_before_declaration_block, "void f(void) { a: { int x; x = 0; } goto a; }");
syntax!(label_multiple, "void f(void) { a: b: c: ; goto b; }");
syntax!(switch_case_default, "void f(int x) { switch (x) { case 1: break; case 2: break; default: break; } }");
syntax!(switch_case_const_expr, "void f(int x) { switch (x) { case 1 + 2: break; } }");
syntax!(switch_fallthrough_and_empty, "void f(int x) { switch (x) { case 1: case 2: ; } }");
syntax!(switch_enum_case, "enum E { A, B }; void f(enum E e) { switch (e) { case A: break; default: break; } }");
...
syntax!(compound_nested_blocks, "void f(void) { { int a; { int b; b = 0; a = b; } } }");
...
syntax!(if_declaration_in_branch_block, "void f(int x) { if (x) { int y; y = 0; } }");
...
syntax!(for_comma_clauses, "void f(void) { int i, j; for (i = 0, j = 9; i < j; i++, j--) ; }");
syntax!(for_nested, "void f(void) { int i, j; for (i = 0; i < 2; i++) for (j = 0; j < 2; j++) ; }");

// ---- 6.6.6 jump statements -----------------------------------------------

syntax!(jump_goto_forward_and_back, "void f(void) { int i; i = 0; back: i++; if (i < 2) goto back; goto fwd; fwd: ; }");
syntax!(jump_continue_break, "void f(void) { int i; for (i = 0; i < 10; i++) { if (i) continue; else break; } }");
...
syntax!(external_multiple_definitions, "int f(void) { return 0; } int g(void) { return f(); }");
syntax!(external_prototype_then_definition, "int f(int); int f(int a) { return a; }");
...
syntax!(external_struct_param_definition, "struct S { int a; }; int f(struct S s) { return s.a; }");
syntax!(knr_definition_no_params, "int f() { return 0; }");
syntax!(knr_definition_with_params, "int f(a, b) int a; int b; { return a + b; }");
...
syntax!(knr_definition_multiple_in_one_decl, "int f(a, b) int a, b; { return a + b; }");
