...
impl Graph {
    ...
    fn add_exit_arrow(&mut self, node: usize, label: &str, l: String) {
        ...
        self.dot.raw(&format!("    {} [shape=point, style=invis, width=0, height=0, label=\"\"];\n", exit_id));

        self.dot.raw(&format!("    {} -> {} [label=\"{}\"];\n", node, exit_id, Dot::escape(&l),));
    }
    ...
    pub fn label_of_transitions(transitions: &[usize]) -> String {
        ...
        while i < len && j < len {
            ...
            if i == j {
                ...
                label += &format!("[{}-{}],", byte_label(transitions[i] as u8), byte_label(transitions[j] as u8));
            }
            j += 1;
            i = j;
        }
    ...
    pub fn from_automaton<T: Automaton + ?Sized>(
        ...
        for (id, node) in automaton.nodes().iter().enumerate() {
            ...
            graph.dot.node(id, shape, &id.to_string(), &format!("{:?}", node.trailing_tags));
