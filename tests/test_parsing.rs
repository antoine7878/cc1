...
/// Grammar coverage: the fixture must be valid C90 (gcc accepts it) and cc1
/// must parse it. Deliberately blind to semantic diagnoses, so these cases
/// track the grammar rather than the completeness of semantic analysis.
...
// ---- 6.5.6 typedef: registration, lookup, scoping -----------------------
...
syntax!(cast_to_function_pointer, "void f(void) { void *p; p = 0; (void)(int (*)(int))p; }");
...
syntax!(type_short_forms, "short a; short int b; signed short c; signed short int d; unsigned short e; unsigned short int f;");
...
// ---- 6.5.5 type names (abstract declarators) -----------------------------
